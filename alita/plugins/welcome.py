...
import Alita.plugins.sql.welcome_sql as sql
from Alita import (DEV_USERS, LOGGER, OWNER_ID, DRAGONS, DEMONS, TIGERS,
                          WOLVES, sw, dispatcher, JOIN_LOGGER)
from Alita.plugins.helper_funcs.chat_status import (
    is_user_ban_protected,
    user_admin,
)
from Alita.plugins.helper_funcs.misc import build_keyboard, revert_buttons
from Alita.plugins.helper_funcs.msg_types import get_welcome_type
from Alita.plugins.helper_funcs.string_handling import (
    escape_invalid_curly_brackets,
    markdown_parser,
)
from Alita.plugins.log_channel import loggable
from Alita.plugins.sql.global_bans_sql import is_user_gbanned
        ...
        if should_welc:
            ...
            if new_mem.id == OWNER_ID:
                update.effective_message.reply_text(
                    "Botun Sahibi Grupa Qatıldı!\nXoş gəldiniz! @samil",
                    reply_to_message_id=reply)
                welcome_log = (f"{html.escape(chat.title)}\n"
                               f"#USER_JOINED\n"
                               f"Botun Sahibi Grupa Qatıldı!\nXoş gəldiniz! @samil")
                continue

            # Welcome Devs
            elif new_mem.id in DEV_USERS:
                update.effective_message.reply_text(
                    "Turan Teamdan biri gəldi!",
                    reply_to_message_id=reply,
                )
                continue
            ...
            # Welcome Sudos
                ...
                update.effective_message.reply_text(
                    "Əjdəha istifadəçimiz gəldi, ehtiyatlı olun!",
                    reply_to_message_id=reply,
                )
                continue
            ...
            # Welcome Support
                ...
                update.effective_message.reply_text(
                    "Turan Team'ın şeytanlarından biri gəldi😈!",
                    reply_to_message_id=reply,
                )
                continue
            ...
            # Welcome Tigers
                ...
                update.effective_message.reply_text(
                    "Oof! Turan Team'in canavarı xoşgəldi!",
                    reply_to_message_id=reply)
                continue
    ...
    if should_goodbye:
        ...
        if left_mem:
            ...
            if left_mem.id == OWNER_ID:
                update.effective_message.reply_text(
                    "Boss qrupu tərk etdi :(..", reply_to_message_id=reply)
                return

            # Give the devs a special goodbye
            elif left_mem.id in DEV_USERS:
                update.effective_message.reply_text(
                    "Turan Team'da görüşərik! :)",
                    reply_to_message_id=reply,
                )
                return
